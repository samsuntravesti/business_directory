				...
				<?php 
					if($_GET['title'] == null & $_GET['region'] == null & $_GET['details'] == null & $organization === Array ()){
						...
						foreach ($organization as $key => $value) {
							//print_r($value);
						echo '<li><div class="info"><a href="'.SITE_URL.'/template/view.php?hvhh='. $value['number'].'">'. $value['name']. '</a><a class="glyphicon glyphicon-pencil" title="Խմբագրել" href="'.SITE_URL.'/template/modification.php?hvhh='. $value['number'] . '" style="float: right; margin-right: -5%"></a><a href="'.SITE_URL.'/template/delete.php?hvhh='. $value['number'].'" class="glyphicon glyphicon-trash" title="Ջնջել" style="float: right; margin-right: 1%"></a></div></li>';
